...
async function getVndToUsdRate() {
    const rate = parseFloat(process.env.PAY2PAY_VND_TO_USD_FALLBACK_RATE);
    if (!isNaN(rate) && rate > 0) {
        return rate;
    }

    logger.warn('PAY2PAY_VND_TO_USD_FALLBACK_RATE is missing or invalid, using default', { FALLBACK_RATE_DEFAULT });
    return FALLBACK_RATE_DEFAULT;
}

/**
 * Convert a VND amount to USD using the configured rate.
...
/**
 * Convert USD to VND using the configured rate.
