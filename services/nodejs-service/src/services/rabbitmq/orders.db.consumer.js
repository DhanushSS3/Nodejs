 ...
 * Mark old ID as 'replaced' and create new 'active' record.
 * Also update the corresponding ID column in the main order table.
 */
async function handleLifecycleIdReplacement(msg) {
  ...
  try {
    ...
    if (user_type) {
      ...
      // id_type from Python can be 'order_id', 'stoploss_id', 'takeprofit_id', 'close_id'
      const updateData = {};
      if (id_type === 'order_id' || id_type === 'provider_order_id') {
      ...
      } else if (id_type === 'close_id') {
        updateData.close_id = String(new_lifecycle_id);
      }
