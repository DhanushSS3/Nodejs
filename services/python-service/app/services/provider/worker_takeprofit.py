...
class TakeProfitWorker:
    ...
    async def connect(self):
        ...
        logger.info("TakeProfitWorker connected. Waiting on %s", TP_QUEUE)
    ...
    async def handle(self, message: aio_pika.abc.AbstractIncomingMessage):
        try:
            ...
            try:
                if await redis_cluster.set(f"takeprofit_confirmed:{order_id}", "1", ex=7 * 24 * 3600, nx=True) is None:
                    logger.info("[TP:skip:idempotent] order_id=%s", order_id)
                    await self._ack(message)
                    return
            except Exception:
                pass
            ...
            if price is None:
                logger.warning("[TP:skip] missing avgpx for order_id=%s", order_id)
                await self._ack(message)
                return
            ...
            try:
                ...
                logger.exception("Failed to publish DB update for takeprofit confirm")

            await self._ack(message)
        except Exception as e:
            logger.exception("TakeProfitWorker handle error: %s", e)
